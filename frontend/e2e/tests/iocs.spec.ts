...
test("ioc lookup shows not found", async ({ page }) => {
  ...
  await page.getByLabel("IoC value").fill("1.2.3.4");
...
test("ioc lookup renders a hit", async ({ page }) => {
  ...
  await page.goto("/iocs");
  await page.getByLabel("IoC value").fill("evil.example");
  await page.getByRole("button", { name: "Lookup" }).click();
  // The defang transform replaces dots; assert the rendered form.
  await expect(page.locator(".lookup-result.found dl.deflist dd code").first()).toHaveText(
    "evil[.]example",
  );
});
...
test("ioc recent list renders rows", async ({ page }) => {
  ...
  await expect(page.locator("table.tbl tbody tr")).toHaveCount(2);
  await expect(page.locator("table.tbl tbody tr").first()).toContainText("203[.]0[.]113[.]1");
});
